...
export default function AddmzModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!name.trim()) return toast.error("가게명을 입력하세요.");
      if (!address.trim()) return toast.error("주소를 입력하세요.");
      ...
      if (categoryNames.length > 0) {
        try {
          ...
          if (fetchCategoryErr) throw fetchCategoryErr;
          ...
          if (missingNames.length > 0) {
            const { data: insertedCategories, error: insertCategoryErr } =
              await supabase
                .from("categories")
            ...
            if (insertCategoryErr) {
              toast.error(
                insertCategoryErr.message ||
                  "새 카테고리를 추가할 권한이 없어 기본값으로 저장됩니다."
              );
            } else {
          ...
          if (categoryIds.length > 0) {
            const { error: linkCategoryErr } = await supabase
            ...
            if (linkCategoryErr) {
              toast.error(
                linkCategoryErr.message ||
                  "카테고리를 연결할 권한이 없어 기본값으로 저장됩니다."
              );
            }
          }
        } catch (catErr) {
          ...
          toast.error(
            catErr.message ||
              "카테고리 정보를 저장하지 못했어요. 기본값으로 저장됩니다."
          );
        }
      }
      ...
      const uploadImageFile = async (file, index, attempt = 0) => {
        ...
        const filePath = `${ownerFolder}/${restaurantId}/${uniqueKey}.${ext}`;
        ...
        if (upErr) {
          const duplicate =
            ...
            /duplicate key value|already exists/i.test(upErr.message);
          if (duplicate && attempt < 3) {
            return uploadImageFile(file, index, attempt + 1);
          }
          console.error("Supabase storage upload error", upErr);
          throw new Error(
            /row-level security/i.test(upErr.message || "")
              ? "이미지 저장 권한이 없어 기본 정보만 저장했습니다."
      ...
      // 이미지 업로드 처리 (선택된 경우)
      if (restaurantId && imageFiles.length > 0) {
        const uploadedUrls = [];
